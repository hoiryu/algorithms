...
const solution = (n, m, arr) => {
	...
	let pl = Math.max(...arr);
	let pr = arr.reduce((acc, curr) => acc + curr, 0);

	while (pl <= pr) {
		const mp = Math.floor((pl + pr) / 2);
		const count = calculate(mp, arr);
...
console.time('걸린 시간');
const log = solution(9, 3, [1, 2, 3, 4, 5, 6, 7, 8, 9]);
