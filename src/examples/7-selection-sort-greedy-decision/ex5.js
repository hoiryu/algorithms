...
export default function solution(s, n, arr) {
	let answer = Array.from({ length: s }).map(() => 0);

	for (let item of arr) {
		let index = -1,
			start = s - 1;
		for (let j = 0; j < s; j++) if (answer[j] === item) index = j;
		if (index !== -1) start = index;
		for (let j = start; j >= 0; j--) answer[j] = answer[j - 1];
