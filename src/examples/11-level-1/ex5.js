...
export default function solution(friends, gifts) {
	let answer = 0;
	gifts = gifts.map(i => i.split(' '));

	const memo = friends.reduce((acc, curr) => {
		acc[curr] = Array.from({ length: 4 }, (_, idx) => {
			return idx <= 1
				? friends.reduce((acc2, curr2) => {
						if (curr === curr2) return acc2;
						acc2[curr2] = 0;
						return acc2;
				  }, {})
				: 0;
	...
	for (let me of friends) {
		for (let [a, b] of gifts) {
			if (me === a) memo[me][0][b]++;
			else if (me === b) memo[me][1][a]++;

			memo[me][2] =
				Object.values(memo[me][0]).reduce((acc, curr) => acc + curr, 0) -
				Object.values(memo[me][1]).reduce((acc, curr) => acc + curr, 0);
		}
	}
	...
	for (let me of friends) {
		for (let friend of friends) {
			...
			if (memo[me][0][friend] > memo[friend][0][me]) {
				memo[me][3]++;
			} else if (
				memo[me][0][friend] === memo[friend][0][me] &&
				memo[me][2] > memo[friend][2]
			) {
				memo[me][3]++;
			}
		}

		answer = Math.max(answer, memo[me][3]);
	}
