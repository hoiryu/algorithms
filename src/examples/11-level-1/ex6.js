 ...
 * 3
 */
export default function solution(bandage, health, attacks) {
	let answer = health;
	const n = Math.max(...attacks.map(([a, b]) => a));
	const memo = Array.from({ length: n }).fill(0);
	let count = 0;

	for (let [a, b] of attacks) memo[a] = b;
	...
	for (let i = 0; i <= n; i++) {
		if (memo[i] > 0) {
			...
			if (count === bandage[0]) answer += bandage[1] + bandage[2];
			else if (count > 0) answer += bandage[1];
		}
